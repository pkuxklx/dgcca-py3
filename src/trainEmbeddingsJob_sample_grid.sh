...
# from previous run
echo "python dgcca_train_harness.py --input ${IN_PATH} --preptrain ${TRAIN_PATH} --preptune ${TUNE_PATH} --output ${EMBEDDINGS_PATH} --model ${MODEL_PATH} --arch \"${architecture}\" --truncparam 500 --k ${k} --rcov ${rcov} ${rcov} ${rcov} ${rcov} --batchSize ${bsize} --l1 ${l1} --l2 ${l2} --vnames ${vnameStr} --activation ${activation} --opt \"${adam_opt}\" --epochs ${numEpochs} --valfreq ${valFreq} --lcurvelog ${HISTORY_PATH} | tee ${LOG_PATH}"
